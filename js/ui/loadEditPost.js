...
async function loadEditPostById(postId) {
    ...
    try {
        let post;
        if (!postId) {
            post = { title: '', body: '', media: { url: '' }, author: { name: '' }, tags: [] };
        } else {
    ...
    <section class="max-w-xl mx-auto mt-6 p-4 bg-white dark:bg-zinc-700 rounded-xl shadow-sm">
        <form id="editPostForm">
            <input name="title" id="title" type="text" placeholder="Title" value="${post.title}" class="w-full p-2 border dark:border-zinc-500 rounded-md bg-zinc-50 dark:bg-zinc-600 dark:text-white dark:placeholder:text-white">
            <textarea name="content" id="content" class="w-full p-2 border dark:border-zinc-500 rounded-md bg-zinc-50 dark:bg-zinc-600 dark:text-white dark:placeholder:text-white" placeholder="What's on your mind?" required="">${post.body}</textarea>

            <!-- Input for image URL -->
            <input type="url" name="imageUrl" id="imageUrl" placeholder="Paste image URL (optional)" value="${post.media.url}" class="mt-2 w-full p-2 border dark:border-zinc-500 rounded-md bg-zinc-50 dark:bg-zinc-600 dark:text-white dark:placeholder:text-white text-sm">

            <input name="tags" id="tags" type="text" placeholder="Tags (comma separated)" value="${post.tags && post.tags.length > 0 ? `${post.tags.join(', ')}` : ''}" class="w-full p-2 border dark:border-zinc-500 rounded-md bg-zinc-50 dark:bg-zinc-600 dark:text-white dark:placeholder:text-white">
